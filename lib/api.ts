...
export const fetchProvinces = async (version: "v1" | "v2") => {
    try {
        const url = version === "v1" ? `${PROVINCES_API_V1}/p` : `${PROVINCES_API_V2}/p`;
        ...
        throw new Error(`Failed to load provinces: ${error.message || "Network error"}`);
    }
};

// Fetch districts for a province
export const fetchDistricts = async (provinceCode: string, version: "v1" | "v2") => {
    if (version === "v2") {
    ...
    try {
        ...
        throw new Error(`Failed to load districts: ${error.message || "Network error"}`);
    }
};
...
export const fetchWards = async (code: string, version: "v1" | "v2") => {
    try {
        ...
        throw new Error(`Failed to load wards: ${error.message || "Network error"}`);
    }
};

// Convert old to new
export const convertOldToNew = async (address: AddressInput): Promise<ConversionResponse> => {
